...
def plot_chain(sampler,
    ...
    plt.savefig('Figs/TOI_{}_plotchain.pdf'.format(toi))
...
def corner(samples,
    ...
    plt.savefig('Figs/TOI_{}_Corner.pdf'.format(toi))
...
def light_curve(lc,
    ...
    plt.savefig('Figs/TOI_{}_modeled.pdf'.format(toi))
...
def rms_plot(lc,
    ...
    plt.yscale('log')
    plt.xscale('log')
    plt.title('RMS vs Bins for TOI {}'.format(toi))